...
Fullscreen is an overlay: it does not change the tiling container layout, and toggling it off restores the underlying tree.
The fullscreen window remains fullscreen when another window is focused or opened in the same workspace.
Entering fullscreen on another window transfers fullscreen to that window.
