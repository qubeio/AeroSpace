...
This qubeio build is installed from source (no Homebrew cask yet).

[source,bash]
----
git clone https://github.com/qubeio/AeroSpace.git
cd AeroSpace
./build-release.sh   # or ./build-debug.sh
----

See `dev-docs/development.md` for dependencies and codesigning.

[#manual-installation]
=== Manual installation (release zip)

. Download the latest available zip from https://github.com/qubeio/AeroSpace/releases[releases page] (when published)
. Unpack zip
...
In total, AeroSpace provides 6 possible layout and orientation combinations:
...
- `h_bsp` horizontal binary space partitioning
- `v_bsp` vertical binary space partitioning
...
BSP recursively splits one window slot for each new window. By default, the split anchor is the
deepest last-child window, which produces a deterministic spiral even when focus moves between
insertions. Set `bsp.insertion-point = 'focused'` to split the focused window instead:

[source,toml]
----
[bsp]
    insertion-point = 'tail' # Possible values: tail|focused
----
