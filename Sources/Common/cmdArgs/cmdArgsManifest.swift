public enum CmdKind: String, CaseIterable, Equatable, Sendable {
    ...
    case resetWorkspaceBsp = "reset-workspace-bsp"
    case resize
    case split
    case subscribe
...
func initSubcommands() -> [String: any SubCommandParserProtocol] {
    ...
    for kind in CmdKind.allCases {
        switch kind {
            ...
            case .debugTree:
                result[kind.rawValue] = SubCommandParser(DebugTreeCmdArgs.init)
            case .debugWindows:
            ...
            case .resetWorkspaceBsp:
                result[kind.rawValue] = SubCommandParser(ResetWorkspaceBspCmdArgs.init)
            case .resize:
